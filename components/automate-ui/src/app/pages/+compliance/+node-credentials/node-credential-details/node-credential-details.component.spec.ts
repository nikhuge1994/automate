...
import { NodeCredential, NodeCredentialTypes, SaveNodeCredential } from 'app/entities/node-credentials/node-credential.model';
...
describe('NodeCredentialDetailsScreenComponent', () => {
  ...
  describe('navigated to details page', () => {
    ...
    it('resets  ssh, winrm and sudo to empty string', () => {
      component.saveSuccessful = true;
      expect(component.resetForm.controls['ssh'].value).toEqual(
        {username: '', password: '', key: ''});
      expect(component.resetForm.controls['winrm'].value).toEqual({username: '', password: ''});
      expect(component.resetForm.controls['sudo'].value).toEqual({password: '', options: ''});
    });
  });
